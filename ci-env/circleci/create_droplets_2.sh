...
sed -i "s/<discovery_url>/${DISCOVERY_URL_ESCAPED}/g" ci-env/circleci/cloud-config_2.yaml

for COREOS_MACHINE in ${COREOS_MACHINE_NAMES_2}
do
  CURL_OUTPUT=$(curl -i -s -X POST https://api.digitalocean.com/v2/droplets \
                ...
                -d "{
                  ...
                  "user_data": "'"$(cat ci-env/circleci/cloud-config_2.yaml | sed 's/"/\\"/g')"'"
                }')
  ...
  then
    ...
    echo ${DROPLET_ID_ACC} > "droplets_${CIRCLE_BUILD_NUM}_2.txt"
  else
