...
########################################################################################################################
#!!
#! @description: This method returns a list of the members of a Safe.
...
#! @input safe_url_id: The unique ID of the Safe used when calling Safe APIs.
#! @input filter: Search for Safe members using the following filters. Multiple filters can be applied using the AND operator.
#!                memberType - returns all members according to the type (user or group)
#!                    Example: memberType eq user
#!                membershipExpired - returns either expired members or members that are not expired.
#!                    Example: membershipExpired eq true
#!                includePredefinedUsers - includes predefined users in the returned list.
#!                    Example: includePredefinedUsers eq true
#!                Optional
#! @input search: Searches according to the Safe name.
#!                Optional
#! @input offset: Offset of the first member that is returned in the collection of results.
#!                Default value: 0
#!                Optional
#! @input sort: Sorts according to the memberName property in ascending order (default) or descending order to control the sort direction.
#!              Valid values: memberName  asc, memberName desc
#!              Optional
#! @input limit: The maximum number of members that are returned.
#!               When used together with the Offset parameter, this value
#!               determines the number of members to return, starting from the first safe that is returned.
#!               Default value: 25
#!               Optional
...
operation:
  ...
  java_action:
    ...
    class_name: io.cloudslang.content.cyberark.actions.safes.GetAllSafeMembers
    method_name: execute
