########################################################################################################################
#!!
#! @description: This flow reads all the releases found in a workspace identified by it's id within a shared space also identified by it's id, both of the given as input parameters. The output of this flow is a JSON containing all existing releases.
#!
...
#! @output status_code: The returned JSON containing information about the modified entities, which could be empty in case of deleting items.
#! @output return_code: The message given by the flow in case an error occured.
#! @output error_message: The code specifying 0 for success or -1 for failure.
#! @output return_result: The code that indicates whether a specific HTTP request has been successfully completed.
#!!#
########################################################################################################################
    ...
    - read_all_releases:
        do:
          io.cloudslang.base.http.http_client_get:
            - url: "${url + '/api/shared_spaces/' + shared_space_id + '/workspaces/' + workspace_id + '/releases'}"
            - auth_type: '${auth_type}'
            - username: '${username}'
            - password:
                value: '${password}'
