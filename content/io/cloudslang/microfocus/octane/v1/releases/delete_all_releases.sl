...
########################################################################################################################
#!!
#! @description: This flow deletes all the releases found in a workspace within a shared space, both identified by their id, given as input parameter.
#!
    ...
    - http_client_delete:
        do:
          io.cloudslang.base.http.http_client_delete:
            - url: "${url + '/api/shared_spaces/' + shared_space_id + '/workspaces/' + workspace_id + '/releases?query=\"id GT 1\"'}"
            - auth_type: '${auth_type}'
            - username: '${username}'
            - password:
                value: '${password}'
