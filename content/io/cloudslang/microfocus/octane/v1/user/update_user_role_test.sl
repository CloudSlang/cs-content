########################################################################################################################
#!!
#! @description: This flow activate a deactivate user
#!
...
#! @input user_id: User Id for update
...
#! @input id_actual_user_role: Role id
#! @input id_new_user_role: New role id
#!
...
flow:
  ...
  inputs:
    ...
    - id_new_user_role
  workflow:
    - create_body_update_user_role:
        do:
          io.cloudslang.microfocus.octane.v1.utils.create_body_update_user_role:
            - id_actual_user_role: '${id_actual_user_role}'
            - id_new_user_role: '${id_new_user_role}'
        publish:
          - body
        navigate:
          - SUCCESS: update_user
    - update_user:
        do:
          io.cloudslang.base.http.http_client_put:
            - url: "${url+'/api/shared_spaces/'+shared_spaces+'/users/'+user_id}"
            - auth_type: anonymous
            - headers: '${header}'
            - body: '${body}'
  ...
  graph:
    steps:
      ...
      update_user:
        ...
        'y': 150
        navigate:
          3a6b255d-e0ee-22dd-93e4-2310a673f276:
            targetId: eb3db23c-b5d3-a0e5-fc36-98d997f80899
            port: SUCCESS
    results:
      SUCCESS:
        eb3db23c-b5d3-a0e5-fc36-98d997f80899:
          x: 700
          'y': 150
