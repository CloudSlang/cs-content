########################################################################################################################
#!!
#! @description: This flow retrieves a list of fields metadata on sharedspace level
#!
...
#! @output total_count: Total numbers of fields
#! @output data: The list containing the fields metadata
#!!#
########################################################################################################################
namespace: io.cloudslang.microfocus.octane.v1.retrieve_metadata
flow:
  name: get_fields_metadata_sharedspaces_test
  inputs:
    ...
    - http_client_get:
        do:
          io.cloudslang.base.http.http_client_get:
            - url: "${url+'/api/shared_spaces/'+shared_spaces+'/metadata/fields'}"
            - auth_type: '${auth_type}'
            - headers: '${header}'
        publish:
          - return_result
  ...
  graph:
    steps:
      ...
      parse_return_result:
        ...
        'y': 150
        navigate:
          61e9e73f-2e67-605b-5bb6-dc3161053c7d:
            targetId: f665fe7d-12e5-9346-5d8d-a919ff460150
            port: SUCCESS
    results:
      SUCCESS:
        f665fe7d-12e5-9346-5d8d-a919ff460150:
          x: 700
          'y': 150
