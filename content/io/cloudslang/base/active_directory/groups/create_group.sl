...
#! @input distinguished_name: The Organizational Unit DN or Common Name DN to add the group to (i.e. OU=OUTest1,DC=battleground,DC=ad).
#! @input group_common_name: The name of new group (its CN).
#! @input group_type: The type of the new group. The groupType values represent: -2147483646 (Security Group - Global), -2147483644 (Security Group - Domain Local), -2147483640 (Security Group - Universal), 2 (Distribution Group - Global), 4 (Distribution Group - Domain Local), 8 (Distribution Group - Universal)
Valid values: -2147483646, -2147483644, -2147483640, 2, 4, 8
Default value: -2147483646
#!                    Optional
#! @input s_am_account_name: The sAMAccountName of the new group. If it contains special characters, these will be
...
#! @output group_distinguished_name: The Distinguished Name of the newly created group.
...
#!
#! @result SUCCESS: Created a new group in Active Directory successfully.
#! @result FAILURE: Failed to create a new group in Active Directory.
#!!#
########################################################################################################################
...
operation: 
  ...
  inputs: 
    ...
    - group_common_name    
    - groupCommonName: 
        default: ${get('group_common_name', '')}  
        required: false 
        private: true 
    - group_type:  
        required: false  
    - groupType: 
        default: ${get('group_type', '')}  
        required: false 
        private: true 
    - s_am_account_name    
    - sAMAccountName: 
  ...
  outputs: 
    ...
    - group_distinguished_name: ${get('groupDistinguishedName', '')} 
