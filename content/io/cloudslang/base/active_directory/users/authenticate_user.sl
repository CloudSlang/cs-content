...
#!
#! @input host: The hostname or IP address of the Active Directory server.
#! @input protocol: The protocol to use when connecting to the Active Directory server. Valid values: HTTP and HTTPS.
#!                  Optional
#! @input username: The user's windows username. The only valid format is domain\username.
#! @input password: The user's password.
#! @input root_distinguished_name: The distinguished name of the root element whose subtree you want to search in (e.g. CN=Users,DC=domain,DC=com).
