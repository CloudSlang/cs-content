      ...
      - get_postgresql_service_user:
          do:
            scripts.powershell_script:
              ...
              - script: >
                  ${'$credential = gwmi -class win32_service | ? {$_.Name -eq \''+ service_name  +'\'} | Select startname -ExpandProperty startname; $credential.TrimStart(\'.\\\');'}
          publish:
            -  return_code
            -  return_result
            -  stderr
            -  exception
  outputs:
    - service_user: ${return_result}
    - return_result
    - exception
    - return_code :  ${"0" if service_user != '' and exception == '' else "-1"}
  results:
    - SUCCESS
    - FAILURE
