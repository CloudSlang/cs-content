...
#!
#! @output response_headers: Jira delete project asynchronously response headers
#! @output status_code: 303 - Returned if the request is successful.
...
#! @result SUCCESS: status_code == 303
#!!#
########################################################################################################################
...
flow:
    ...
    - http_client_post:
        do:
          io.cloudslang.base.http.http_client_post:
            - url: "${url + '/rest/api/3/project/' + project_id_or_key + '/delete'}"
            - auth_type: basic
            - username: '${username}'
            - password:
                value: '${password}'
  ...
  outputs:
    - response_headers: '${response_headers}'
    - status_code: '${status_code}'
    - error_message: "${error_message if error_message != '' else (return_result if status_code != '303' else '')}"
    - return_result: "${return_result if status_code == '303' else ''}"
    - return_code: '${return_code}'
  results:
    - FAILURE
    - SUCCESS
