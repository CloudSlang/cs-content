########################################################################################################################
#!!
#! @description: Sets the default columns for a given account id.
#!
#! @input url: URL to which the call is made.
#! @input username: Username used for URL authentication
#! @input password: Password used for URL authentication
#! @input account_id: Account id to set the default columns to
#! @input columns: List of columns delimeted by ,
#!                 ex: summary,description
...
#!
#! @output response_headers: Jira set default columns response headers
...
#!                      500 - Returned if an invalid issue table column ID is sent.und.
    ...
    - http_client_put:
        do:
          io.cloudslang.base.http.http_client_put:
            ...
            - body: "${'[' + \"\".join(map(lambda column: '\"' + column + '\",', columns.split(\",\")))[:-1] + ']'}"
            - content_type: '*/*'
