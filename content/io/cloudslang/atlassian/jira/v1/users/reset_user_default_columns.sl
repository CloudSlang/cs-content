########################################################################################################################
#!!
#! @description: Resets the default issue table columns for the user to the system default.
#!
#! @input url: Jira domain
#! @input username: Username for the API authenticator
#! @input password: Password for the API authenticator
#! @input account_id: The account ID of the user, which uniquely identifies the user across all Atlassian products. For example, 5b10ac8d82e05b22cc7d4ef5.
...
#! @output error_message: The API call error or the retrieved entity error as JSON. Code 401 returned if the authentication credentials are incorrect or missing. Code 403 returned if the user does not have the necessary permission.  Code 404 returned if the user is not found.
#!
#! @result FAILURE: Operation failed
#! @result SUCCESS: Columns reseted succesfully
#!!#
########################################################################################################################
namespace: io.cloudslang.atlassian.jira.v1.users
flow:
  name: reset_user_default_columns
  inputs:
    - url
    - username:
        required: false
    ...
    - http_client_get:
        do:
          io.cloudslang.base.http.http_client_get:
            - url: "${url + '/rest/api/3/user/columns'}"
            - auth_type: null
            - username: '${username}'
            - password:
                value: '${password}'
  ...
  graph:
    steps:
      http_client_get:
        ...
        'y': 160
        navigate:
          08ce3ee2-3f24-feb1-8074-f5bb06f39390:
