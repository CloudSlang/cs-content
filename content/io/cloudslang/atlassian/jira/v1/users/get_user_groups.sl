...
#!
#! @output group_names: A list of group names separated by comma.
...
#!
#! @result SUCCESS: User groups retrieved successfully.
#! @result FAILURE: Failed to retrieve user groups.
#!!#
########################################################################################################################
...
flow:
  ...
  workflow:
    - http_client_get:
        do:
          io.cloudslang.base.http.http_client_get:
            - url: '${url+"/rest/api/3/user/groups?accountId="+account_id}'
            - auth_type: basic
            - username: '${username}'
            - password:
                value: '${password}'
        ...
        publish:
          ...
          - return_result
        navigate:
          - SUCCESS: get_user_groups_group_names
    ...
    - test_for_http_error:
        ...
        navigate:
          - FAILURE: FAILURE
    - get_user_groups_group_names:
        do:
          io.cloudslang.atlassian.jira.v1.utils.get_user_groups_group_names:
            - return_result: '${return_result}'
        publish:
          - group_names
        navigate:
  ...
  graph:
    steps:
      ...
      get_user_groups_group_names:
        ...
        'y': 125
        navigate:
          d848bd7c-3486-d22b-7666-dcf469f3cbfb:
            targetId: 0d6857f2-5171-e4c3-9393-b2dcc854101e
            port: SUCCESS
      test_for_http_error:
        x: 400
        'y': 375
        navigate:
          f8d37f37-c7e1-c6e0-cfd5-e0ae9b670fa5:
            targetId: 3cc8c53f-edc6-b54a-f08c-e38daf11797f
            port: FAILURE
    results:
      SUCCESS:
        0d6857f2-5171-e4c3-9393-b2dcc854101e:
          x: 700
          'y': 125
      FAILURE:
        3cc8c53f-edc6-b54a-f08c-e38daf11797f:
          x: 700
          'y': 375
