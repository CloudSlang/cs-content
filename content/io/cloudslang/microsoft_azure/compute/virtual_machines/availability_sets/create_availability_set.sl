...
########################################################################################################################
#!!
#! @description: Performs an HTTP request to create an availability set for virtual machines
#!
...
#!                                Default: "false"
#!
#! @output output: json response with information about the availability set
#! @output status_code: 201 if request completed successfully, others in case something went wrong
#! @output error_message: If the availability set could not be created the error message will be populated with a response,
#!                        empty otherwise
#!
#! @result SUCCESS: Availability set created successfully.
#! @result FAILURE: There was an error while trying to create the availability set.
#!!#
########################################################################################################################
...
flow:
  ...
  inputs:
    - url: ${'https://management.azure.com/subscriptions/' + subscription_id + '/resourceGroups/' + resource_group_name + '/providers/Microsoft.Compute/availabilitySets/' + availability_set_name + '?api-version=2016-03-30'}
    - subscription_id
    - location
    - auth_token
    - resource_group_name
    ...
    - http_client_put:
        do:
          http.http_client_put:
            - url
            - body: >
                ${'{"name":"' + availability_set_name + '","type": "Microsoft.Compute/availabilitySets","location":"' + location + '","tags":{},"properties":{"platformUpdateDomainCount":5,"platformFaultDomainCount":3}}'}
