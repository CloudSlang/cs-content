...
########################################################################################################################
#!!
#! @description: Performs an HTTP request to delete an availability set
...
#!                                Default: "false"
#!
#! @output output: json response with information of the deleted availability
#! @output status_code: 202 if request completed successfully, others in case something went wrong
#! @output error_message: If a availability is not found the error message will be populated with a response,
#!                        empty otherwise
#!
#! @result SUCCESS: Availability set deleted successfully.
#! @result FAILURE: There was an error while trying to delete the availability set.
#!!#
########################################################################################################################
...
flow:
  name: delete_availability_set

  inputs:
    - url:
        default: ${'https://management.azure.com/subscriptions/' + subscription_id + '/resourceGroups/' + resource_group_name + '/providers/Microsoft.Compute/availabilitySets/' + availability_set_name + '?api-version=2016-09-01'}
    - auth_token
    - resource_group_name
