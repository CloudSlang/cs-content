...
#! @input pod_json_body: The JSON input of the pod to be replaced.
...
#! @output pod_json: The details of replaced pod.
#!!#
########################################################################################################################
...
flow:
  ...
  inputs:
    ...
    - trust_password:
        ...
        sensitive: true
  workflow:
    - api_to_replace_kubernetes_pod:
        worker_group:
          value: '${worker_group}'
          override: true
        do:
          io.cloudslang.base.http.http_client_put:
            - url: "${'https://'+kubernetes_host+':'+kubernetes_port+'/api/v1/namespaces/'+namespace+'/pods/'+pod_name}"
            - auth_type: anonymous
