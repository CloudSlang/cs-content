...
#                             - valid: basic, form, springForm, digest, ntlm, kerberos, anonymous (no authentication) - Default: basic
#       - preemptiveAuth - optional - if this field is 'true' authentication info will be sent in the first request, otherwise a request with no authentication info will be made and if server responds
#                                   - with 401 and a header like WWW-Authenticate: Basic realm="myRealm" only then the authentication info will be sent - Default: true
#       - username - optional - user name used for URL authentication; for NTLM authentication, the required format is 'domain\user'
...
#       - kerberosLoginConfFile - optional - login.conf file needed by the JAAS framework with the content similar to the one in examples
...
#       - connectionsMaxTotal - maximum limit of connections in total - Default: 2
#       - headers - list containing the headers to use for the request separated by new line (CRLF); header name - value pair will be separated by ":"
#                 - Format: According to HTTP standard for headers (RFC 2616) - Examples: Accept:text/plain
#       - responseCharacterSet - character encoding to be used for the HTTP response - Default: ISO-8859-1
#       - destinationFile - optional - absolute path of a file on disk where to save the entity returned by the response
...
#       - queryParamsAreURLEncoded - specifies whether to encode  (according to the url encoding standard) the queryParams - Default: false
#       - queryParamsAreFormEncoded - specifies whether to encode the queryParams in the form request format or not -  Default: true
#       - formParams - optional - input needs to be given in form encoded format and will set the entity to be sent in the request.
#                               - Examples: input1=value1&input2=value2. (The client will send: input1=value1&in+put+2=val+u%0A+e2)
#       - formParamsAreURLEncoded formParams will be encoded  (according to the url encoding standard) if this is 'true' - Default: false
...
#       - body - string to include in body for HTTP POST operation. If both sourceFile and body will be provided,
#       - the body input has priority over sourceFile; should not be provided for method=GET, HEAD, TRACE
#       - contentType - content type that should be set in the request header, representing the MIME-type of the
#                     - data in the message body - Default: text/plain
#       - requestCharacterSet - character encoding to be used for the HTTP request body; should not be provided for method=GET, HEAD, TRACE - Default: ISO-8859-1
#       - multipartBodies - optional a name=textValue list of pairs separated by "&"; will also take into account the "contentType" and "charset" inputs
#       - multipartBodiesContentType - each entity from the multipart entity has a content-type header; you can only specify it once for all the parts and it is the only way to change
#                                    - the characterSet of the encoding - Default: text/plain; charset=ISO-8859-1
...
#                                   - Default: application/octet-stream - Examples: image/png,text/plain
#       - multipartValuesAreURLEncoded - set this to 'true' if the bodies may contain the "&" and "=" - Default: false
#       - chunkedRequestEntity - optional - data is sent in a series of "chunks" - Valid: true/false
#       - method - HTTP method used; This is a required input.
#       - httpClientCookieSession - optional - session object that holds the cookies if the useCookies input is true
#       - httpClientPoolingConnectionManager - GlobalSessionObject that holds the http client pooling connection manager
#
#   Outputs:
