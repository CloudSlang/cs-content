...
flow:
  ...
  workflow:
    ...
    - create_file_and_copy_it_to_src_host:
        do:
          base_cmd.run_command:
            - command: "'echo ' + text_to_check + ' >> ' + scp_file + ' && cat ' + scp_file + ' && scp -P ' + first_scp_host_port + ' -o \"StrictHostKeyChecking no\" -i ' + key_name  + ' ' + scp_file + ' ' + scp_username + '@' + host + ':' + container_path + scp_file"
        navigate:
